package com.ntuzy.hadoop.mapreduce.secondarysort;
...
//		FileSystem fileSystem = FileSystem.get(URI.create(args[1]), configuration);
		FileSystem fileSystem = FileSystem.get(URI.create("hdfs://node01:8020/sort"), configuration);
		//生产中慎用
		if (fileSystem.exists(new Path("hdfs://node01:8020/sort"))) {
			fileSystem.delete(new Path("hdfs://node01:8020/sort"), true);
		}

//		FileInputFormat.setInputPaths(job, new Path(args[0]));
		FileInputFormat.setInputPaths(job, new Path("hdfs://node01:8020//salary.txt"));
...
//		job.setMapOutputValueClass(NullWritable.class);
		...
		FileOutputFormat.setOutputPath(job, new Path("hdfs://node01:8020/sort"));
